...
public class AggregationMinStrategy implements AggregationStrategy {
    ...
    private final static Logger LOG = Logger.getLogger(AggregationMinStrategy.class.getName());
    ...
    public ValResult evaluate(ValNode operandChild, List<ValResult> resultsGrouped) {
        ...
        try {
                ...
                for (ValResult result : resultsGrouped) {
                    if(!isFirstResult){
                        ...
                        if(valueDataType == null || (dataType != null && valueDataType.getDataTypeId() != dataType.getDataTypeId())){
                            LOG.log(Level.SEVERE,"Operação de min realizada com diferentes tipos de dados, na regra " + operandChild.getOperationVersion().getOperationVID());
                            return null;
                        }
                    }
            ...
            LOG.log(Level.SEVERE,"Ocorreu um erro ao realizar a operação de min no nó " + operandChild.getOperationVersion().getOperationVID(),e);
        }
