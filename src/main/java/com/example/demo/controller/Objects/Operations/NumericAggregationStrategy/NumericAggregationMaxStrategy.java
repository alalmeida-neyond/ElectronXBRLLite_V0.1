...
private final Logger LOG = Logger.getLogger(NumericAggregationMaxStrategy.class.getName());
            ...
            if(resultsGrouped != null && !resultsGrouped.isEmpty()){
                for(Map.Entry<ValNode, ValResult> pair : resultsGrouped){
                    if(!isFirstResult){
                        ...
                        if(valueDataType == null || (dataType != null && dataType.getDataTypeId() != Constants.DATATYPENOTAPPLICABLE && valueDataType.getDataTypeId() != Constants.DATATYPENOTAPPLICABLE && valueDataType.getDataTypeId() != dataType.getDataTypeId())){
                            LOG.log(Level.SEVERE,"Operação de max realizada com diferentes tipos de dados, na regra " + pair.getKey().getOperationVersion().getOperationVID());
                            return null;
                        }
                    }
